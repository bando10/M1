...
int main(int argc, char *argv[]) {
  ...
  scanf("%d", &nb_msg);
  ssize_t res = send(ds, &nb_msg, sizeof(int) +1, 0);
  if(res < 0){
      perror("Client : erreur a l'envoi : ");
      exit(1);
  } else if(res == 0) {
   printf("%d messages a envoyer", nb_msg);
  }
  ...
  for(int i=1; i<=nb_msg; i++){
      ...
      printf("Message n°%d : \n", i);
      ssize_t res2 = send(ds, &m, strlen(m) +1, 0);
      if (res2 < 0) {
         perror("res < 0 \n");
      }
      printf("Client : nombre d'octets envoyes : %li \n", res2);
  }
